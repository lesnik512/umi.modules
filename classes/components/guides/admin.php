...
class GuidesAdmin {
    ...
    public function config() {
        ...
        $active_guides = $this->module->list_guid;
        if($mode == "do") {
            if(sizeof($new_types = getRequest('type'))){
                ...
                $this->module->list_guid = $res_arr;
                $regedit->setVar("//modules/guides/list", implode(',',$res_arr));
            } else{
                $this->module->list_guid = array();
                $regedit->setVar("//modules/guides/list",'');
            }
            $this->chooseRedirect($this->module->pre_lang . '/admin/guides/lists/' );
        }
    ...
    public function lists() {
        ...
        if(!$type) $type = $this->module->list_guid[0];
        if(!$type){
            $this->chooseRedirect($this->module->pre_lang . '/admin/guides/config/' );
        }
        $guide = selector::get('object-type')->id($type);
        if ($guide) $this->setHeaderLabel(getLabel('header-guides-lists') . ' "' . $guide->getName() . '"');
        //if($this->module->ifNotXmlMode()) $this->doData();
        ...
        foreach($sel->result as $object){
            ...
            $line_arr['edit-link'] = $this->module->getObjectEditLink($object->id);
            if(getRequest('viewMode') == 'full') {
    ...
    public function add() {
        ...
        if($mode == "do") {
            ...
            $this->chooseRedirect($this->module->pre_lang . '/admin/guides/edit/' . $object->getId() . '/');
        }
